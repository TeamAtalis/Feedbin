...
Rails.application.routes.draw do
  ...
  get :health_check, to: proc { |_env| [200, {}, ['OK']] }
  get :version, to: proc { |_env| [200, {}, [File.read('REVISION')]] }
  ...
  resources :tags, only: %i[index show update destroy edit]
  resources :billing_events, only: [:show]
  resources :in_app_purchases, only: [:show]
  resources :app_store_notifications, only: [:show]
  resources :password_resets
  resources :sharing_services, path: 'settings/sharing', only: %i[index create update destroy]
  resources :actions, path: 'settings/actions', only: %i[index create new update destroy edit]
  ...
  resources :saved_searches, only: %i[show update destroy create edit new] do
    collection do
      get :count
    end
  end
  ...
  resources :supported_sharing_services, only: %i[create destroy update] do
    member do
      get :oauth_response
      get :oauth2_response
      get :autocomplete
      match 'share/:entry_id', to: 'supported_sharing_services#share', as: :share, via: %i[get post]
    end
  end

  resources :subscriptions, only: %i[index edit create destroy update]
  ...
  resources :feeds, only: %i[index edit create update] do
    patch :rename
  ...
  resources :entries, only: %i[show index destroy] do
    member do
      post :content
  ...
  namespace :settings do
    resources :subscriptions, only: %i[index edit destroy update] do
      collection do
  ...
  post 'settings/subscription_view_mode/:feed_id', as: :settings_subscription_view_mode,
                                                   to: 'settings#subscription_view_mode'
  ...
  constraints subdomain: 'api' do
    namespace :api, path: nil do
      ...
      namespace :podcasts do
        namespace :v1 do
          resources :feeds, only: :show
          resources :subscriptions, only: %i[index create update destroy]
          namespace :queued_entries do
            resource :bulk, controller: :bulk, only: [:update]
          end
          resources :queued_entries, only: %i[index create update destroy]
      ...
      namespace :v2 do
        resources :feeds, only: [:show] do
          resources :entries, only: %i[index show], controller: :feeds_entries
        end
        ...
        resources :actions, only: %i[index create update] do
          member do
            get :results
          end
          collection do
        ...
        resources :subscriptions, only: %i[index show create destroy update]

        resources :favicons, only: [:index]
        resources :icons, only: [:index]
        resources :taggings, only: %i[index show create destroy]
        resources :recently_read_entries, only: %i[index create]
        ...
        resources :unread_entries, only: %i[index show create]
        ...
        resources :starred_entries, only: %i[index show create]
        ...
        resources :saved_searches, only: %i[index show create destroy update]
